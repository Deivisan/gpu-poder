 ...
 *
 * Extend C11 mesa_call_once to support context parameter
 */
...
#define UTIL_ONCE_FLAG_INIT { false, MESA_ONCE_FLAG_INIT }

/**
 * This is used to optimize the call to mesa_call_once out when the func are
...
util_call_once(util_once_flag *flag, void (*func)(void))
{
   if (unlikely(!p_atomic_read_relaxed(&flag->called))) {
      mesa_call_once(&flag->flag, func);
      p_atomic_set(&flag->called, true);
   }
}

/**
 * @brief Wrapper around mesa_call_once to pass data to func
 */
void
util_call_once_data_slow(mesa_once_flag *once, util_call_once_data_func func, const void *data);
